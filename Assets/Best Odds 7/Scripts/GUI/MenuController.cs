...
public class MenuController : MonoBehaviour {
	...
	public GUI_Object MainMenu;
	public GUI_Object ThemeMenu;
	private bool ThemeMenuOpen = false;

	public GUI_Object ThemeButton;

	public ThemeButton selectedTheme;
	...
	[SerializeField]
	private GUI_Object TitleClouds1;

	[SerializeField]
	private GUI_Object TitleClouds2;
	...
	public void StartSinglePlayer()
	{
		...
		StartCoroutine(EasyBtn.AnimateTo(EasyBtn.targetPos, .8f));
		StartCoroutine(MedBtn.AnimateTo(MedBtn.targetPos, 1.2f));
		StartCoroutine(HardBtn.AnimateTo(HardBtn.targetPos, 1.2f));

		Singleplayer_MenuButton.Close();
	}

		public void ChooseLocalMultiplayer()
	{
		//Application.LoadLevel("Main");
		StartCoroutine(EasyBtnLM.AnimateTo(EasyBtnLM.targetPos, .8f));
		StartCoroutine(MedBtnLM.AnimateTo(MedBtnLM.targetPos, 1.2f));
		StartCoroutine(HardBtnLM.AnimateTo(HardBtnLM.targetPos, 1.2f));

		Multiplayer_MenuButton.Close();
	}
	...
	public void ShowThemeMenu() 		//Refactor needed -- temp
	{
		if(ThemeMenuOpen)
		{
			HideThemeMenu();
		...
		{
			Debug.Log("PLSSSS");
			ThemeButton.GetComponent<NoGravity>().enabled = false;
			ThemeButton.gameObject.transform.rotation = Quaternion.identity;
			GUI_Controller.instance.RotateObjectForward(1f,ThemeButton.gameObject);
			ThemeMenu.gameObject.SetActive(true);
			MainMenu.gameObject.SetActive(false);
			//StartCoroutine(ThemeMenu.AnimateTo(ThemeMenu.targetPosMarker.transform.position, .1f));
			ThemeMenuOpen = true;
		}
	...
	{
		//StartCoroutine(MainMenu.AnimateTo(MainMenu.targetPos, 1f));
		MainMenu.gameObject.SetActive(true);
		ThemeMenu.gameObject.SetActive(false);
		GUI_Controller.instance.RotateObjectForward(1f,ThemeButton.gameObject);
		//ThemeButton.GetComponent<NoGravity>().enabled = false;
		ThemeMenuOpen = false;
	...
	public void LoadMenuTheme(Theme theme)
	{
		// foreach(TileSpawner spawner in Tile1Spanwers)
		// {
		// 	spawner.Tile = theme.Tile1MM;

		// }

		// foreach(TileSpawner spawner in Tile2Spanwers)
		// {
		// 	spawner.Tile = theme.Tile2MM;	
		// }

		// foreach(TileSpawner spawner in Tile3Spanwers)
		// {
		// 	spawner.Tile = theme.Tile3MM;			
		// }
	...
	{
		CloseAllMenus(true);
		//StartCoroutine(TitleClouds1.AnimateTo(TitleClouds1.targetPosMarker.transform.position, 1f));
		//StartCoroutine(TitleClouds2.AnimateTo(TitleClouds2.targetPosMarker.transform.position, 1f));
