...
public class InAppPurchasesController : MonoBehaviour , IStoreListener {
	...
	public RectTransform energyStoreMarker;
	...
	public void LoadStore()
	{
		...
		{
			//Debug.LogWarning("Loading  energy pass object");
		...
		{
			//Debug.LogWarning("Loading coin object");
	...
	public void LockEnergyShopButtons()
	{
		foreach(ShopItem item in energyPasses)
		{
			item.GetComponent<Button>().interactable=false;
		}

		energyRefill.GetComponent<Button>().interactable=false;
	}

	public void UnlockEnergyShopButtons()
	{
		foreach(ShopItem item in energyPasses)
		{
		...
		energyRefill.GetComponent<Button>().interactable=true;
	}
	...
	// This is invoked manually on Start to initialize UnityIAP
	public void InitializePurchasingIOS() {
		// If IAP is already initialized, return gently
		if (IsInitialized) return;
		...
		} else if (Application.platform == RuntimePlatform.IPhonePlayer) 
		{
			...
			(
				new ValidateIOSReceiptRequest {
					CurrencyCode = e.purchasedProduct.metadata.isoCurrencyCode,
					PurchasePrice = Decimal.ToInt32 (e.purchasedProduct.metadata.localizedPrice * 100),
					ReceiptData = payload
				},
				(ValidateIOSReceiptResult result) => PurchaseComplete (),
				error => Debug.Log ("iOS validation failed: " + error.GenerateErrorReport ())
			);
		}
