...
public class AI_Player : MonoBehaviour {
    ...
    public int movesMadeThisTurn=0;
    ...
    private void PlayFirstTile()
    {
        ...
        foreach(GridTile tile in CurrentHand)
        {
            if(!isEven(tile.value))
            {
                ...
                tile.GetComponent<GUI_Object>().targetPos = GameMaster.instance.objGameGrid[BoardController.instance.GRID_CENTER, BoardController.instance.GRID_CENTER].transform.position + new Vector3(0, GameMaster.TILE_PLACEMENT_OFFSET, -1);
    ...
    private bool firstExchange=false;
    ...
    public IEnumerator WaitLoop()
    {
        ...
        {
            if(GameMaster.instance.turnCounter > 6 && GameMaster.instance.playerScores[0] < GameMaster.instance.playerScores[1])
            {
                noValidMoves=true;
            }
        }
        ...
        if(ApplicationModel.GAME_DIFFICULTY < 19 && movesMadeThisTurn ==1)
        {
            noValidMoves=true;
        } else if ( ApplicationModel.GAME_DIFFICULTY < 30 && movesMadeThisTurn > 1)
        {
            noValidMoves=true;
        }
        ...
        if (CurrentHand.Count == 0 || noValidMoves == true)
        {
            ...
            {
                Debug.LogError("AI HAND Exchange");

                // if(ApplicationModel.TUTORIAL_MODE && !firstExchange && !BoardController.instance.firstEvenTileRequired) //and not already done
                // {
                //     firstExchange=true;
                //     Tutorial_Controller.instance.tutorialIndex=43;
        ...
        foreach (int x in Criteria.xSpaces)
        {
            ...
            foreach (GridTile tile in CurrentHand) //GameMaster.instance.Player1Hand)
            {
                ...
                if ((tile.value == currentTileVal) && tilePlayed == false)
                {
                    ...
                    tile.GetComponent<GUI_Object>().targetPos = GameMaster.instance.objGameGrid[x, y].transform.position + new Vector3(0, GameMaster.TILE_PLACEMENT_OFFSET, -1);
        ...
        if(Move.Correction != null)
        {
            ...
            foreach (GridTile tile in CurrentHand) 
            {
                if (Move.Correction.tileValue == tile.value)
                {
                    ...
                    tile.GetComponent<GUI_Object>().targetPos = GameMaster.instance.objGameGrid[Move.Correction.tileX, Move.Correction.tileY].transform.position + new Vector3(0, GameMaster.TILE_PLACEMENT_OFFSET, -1);
