...
public class AccountInfo : MonoBehaviour {
	...
	public static string ITEM_LEVELPACKBUNDLE = "Level Pack";
	...
	private static void OnLoginSuccess(LoginResult result)
	{
		...
		MenuController.instance.userIDtxt.text="User ID: " + result.PlayFabId;
	...
	static void OnAccountInfoSuccess(GetPlayerCombinedInfoResult result)
	{
		...
		if(Instance != null)
		{
			...
			if(scene.name != "Main")
			{
				MenuController.instance.ReferralController.InitiateReferalPanel();
				Instance.StartCoroutine(Instance.UpdateAccountInfo(60f));
			}
	...
	private IEnumerator UpdateAccountInfo(float dealy)
	{
	...
	public static void UpdateUIContent(GetPlayerCombinedInfoResultPayload info)
	{
		...
		MenuController.instance.CurrencyUI.CalculateMinutesUntilFullEnergy();
	...
	public static void SetUpAccountChallengeData() {
		...
		result =>  MenuController.instance.StartTutorial(),
		error => {
	...
	public static void GetPlayerData(string playfabId) {
		PlayFabClientAPI.GetUserData(new GetUserDataRequest() {
			...
			else
			{
				...
				{
					Debug.Log("Set up new user account");
					SetUpAccount();
...
//			Debug.Log("daily challenge leaderboard retireved");
	...
	public static void OnInventorySuccess(GetUserInventoryResult result)
	{
		...
		if(ApplicationModel.RETURN_TO_WORLD==-1 && Database.Instance.CatalogTileSkins.Count==0)
		{
			//Debug.Log("CallingGGGGG dody cataog load after get inventory");
			Database.LoadCatalog();
		}

		MenuController.instance.UnlockablesController.AdjustTileStoreCollectionOffset();
	}
	...
	{
		MenuController.instance.setNameButton.SetTrigger("pressBtn");
		AccountInfo.Instance.Info.PlayerProfile.DisplayName=name;
		PlayFabClientAPI.UpdateUserTitleDisplayName(new UpdateUserTitleDisplayNameRequest() {
			...
			error => {
				...
				Debug.Log(error.GenerateErrorReport());
				nameErrorText.text=error.GenerateErrorReport();
				ResetNameButton();
		});
	...
	private void ResetNameButton()
	{
		MenuController.instance.setNameButton.SetTrigger("resetBtn");
	...
	public void OnNameSetComplete()
	{
		...
		MenuController.instance.StartMenuTutorial2();
	...
	{
		Debug.Log("Successfully added acount data");
		ApplicationModel.TUTORIAL_MODE=true;
		MenuController.instance.StartTutorial();
	...
	void OnCheckInCallback(ExecuteCloudScriptResult result)
	{
		...
		if(grantedItems != null && grantedItems.Count > 0)
		{
			...
			MenuController.instance.dailyLoginPanel.SetActive(true);
